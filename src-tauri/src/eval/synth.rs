...
pub fn build(data_dir: &Path, prompts: &[Prompt]) -> Result<usize, String> {
     ...
     -> Result<(), String> {
        ...
        manifest::append(
            ...
            &Clip {
                ...
                split: Split::Dev,
                categories: vec![prompt
                    .map(|p| p.category.clone())
                    .unwrap_or_else(|| "non_speech".into())],
    ...
    for prompt in prompts.iter().filter(|p| !p.is_non_speech()) {
        ...
        *n += 1;
        ...
        tts(
            &prompt.text,
            &data_dir.join(format!("{AUDIO_DIR}/{id}.wav")),
        )?;
        add(
            id,
            &prompt.text,
            Expected::Speech,
            Some(prompt),
            &format!("tts-{}", TTS_VOICE.to_lowercase()),
        )?;
    }

    write_generated(
        &data_dir.join(format!("{AUDIO_DIR}/synth-silence.wav")),
        2_000,
        |_| 0,
    )?;
    add(
    ...
    write_generated(
        &data_dir.join(format!("{AUDIO_DIR}/synth-noise.wav")),
        2_000,
        |_| {
    ...
    write_generated(
        &data_dir.join(format!("{AUDIO_DIR}/synth-clicks.wav")),
        3_000,
        |i| {
            let phase = i % 2_400;
            if phase < 40 {
                (12_000 - phase as i32 * 300) as i16 * if phase % 2 == 0 { 1 } else { -1 }
            } else {
                0
            }
        },
