...
pub fn recover_meeting_dir(meeting_dir: &Path) -> Result<Vec<RecoveredTrack>, String> {
    ...
    let mut track_dirs: Vec<PathBuf> = entries
        .flatten()
...
fn recover_track_dir(track_dir: &Path) -> Result<RecoveredTrack, String> {
    ...
    if let Some(sidecar) = &mut sidecar {
        for chunk in sidecar
            .chunks
            .iter_mut()
            .filter(|c| c.status == ChunkStatus::Open)
        {
    ...
    for path in files.flatten().map(|entry| entry.path()) {
        ...
        let listed = sidecar
            .as_ref()
            .is_some_and(|s| s.chunks.iter().any(|c| c.file == file));
        if !listed {
            orphans.push(OrphanChunk {
                n_frames: frames_on_disk(&path)?,
                file,
            });
        }
    }
...
mod tests {
    ...
    fn record_then_die(tmp: &TempDir, kind: TrackKind, frames_in_open_chunk: usize) -> FakeLedger {
        ...
        writer
            .write(&vec![0.3; 2_000 + frames_in_open_chunk])
            .unwrap();
    ...
    fn a_kill_mid_chunk_recovers_the_right_frame_count() {
        ...
        assert_eq!(
            records[2].n_frames, 0,
            "the store never heard how long it got"
        );
        ...
        assert_eq!(
            fs::metadata(&open_path).unwrap().len(),
            247,
    ...
    fn missing_and_empty_files_are_zero_frames() {
        ...
        let repaired = recover_chunks(&records, tmp.path()).unwrap();
        let frames: Vec<(&str, u64)> = repaired
            .iter()
            .map(|r| (r.id.as_str(), r.n_frames))
            .collect();
        assert_eq!(
            frames,
            [
                (records[1].id.as_str(), 1_000),
                ("empty", 0),
                ("missing", 0)
            ]
        );
    ...
    fn a_meeting_directory_recovers_without_the_database() {
        ...
        assert_eq!(
            ...
            [Some(TrackKind::Mic), Some(TrackKind::System)]
        );
        ...
        assert_eq!(
            mic.recovered[0].start_ms, 125,
            "2000 frames in: the anchor survived the crash"
        );
        assert!(mic.orphans.is_empty());
        let statuses: Vec<ChunkStatus> = mic
            .sidecar
            .as_ref()
            .unwrap()
            .chunks
        ...
        assert!(recover_meeting_dir(&tmp.path().join("no-such-meeting"))
            .unwrap()
            .is_empty());
    }
